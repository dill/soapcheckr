...
We now have a list of dataframes split by  each polygon's x and y coordinates that have had the `id` column removed. We then need to create a vector that is `id` number of each polygon. In this case its 1-5, we can use `length()` to easily create the end of our numerical vector. 
...
#### Step 4: Make knots for a more complex boundary using [{sf}](https://r-spatial.github.io/sf/)
...
Next we will create our knot dataframe by extracting the lon and lat of each point and then dropping the `geometry` and selecting our `lon` and `lat` columns. 
...
```
Then we will assess the data to evaluate distribution model should fit the data to. We will use functions from [{fitdistrplus}](https://cran.r-project.org/web/packages/fitdistrplus/index.html). 
```{r check skewness and kurtosis}
depths <- sissabagama_bath$depth
descdist(depths)
```
Skewness and kurtosis of our example data indicates that a model using a Gamma distribution will likely fit. 
```{r check distr}
fit_gamma <- fitdist(depths, distr = "gamma", method = "mme")
plot(fit_gamma)
```

We can see the depth data will likely fit a gamma distribution and therefore our GAM will use a Gamma error distribution.

### Step 6: Run the model 
Prior to running our GAM with a soap film smoother we need to add one last thing to our boundary list. We need to add the variable `f` to every boundary polygon within our boundary list. This variable indicates to the soap-film smoother that our response variable is `0` right at the boundary, otherwise the soapfilm smoother does not know what to do when it hits the boundary.  
...
We can evaluate the main and individual effects of the model using `anova()` and `summary()`
...
ggplot() +
  ...
  labs(x = "Longitude", 
       y = "Latitude")
```
Congratulations we have made a soap film GAM that takes in account the boundaries within the lake to estimate the bathymetry of the lake. For this example I used bathymetry, but you can use this workflow to model any type of variable (e.g., fish and/or animal movement, wind speed, water quality, ect.)
